...
//------------------------------------------------------------------------------
yad64::address_t ELF64::entry_point() {
	read_header();
...
void ELF64::read_header() {
	if(header_ == 0) {
		...
		if(!yad64::v1::debugger_core->read_bytes(region_.start(), header_, sizeof(Elf64_Ehdr))) {
...
//------------------------------------------------------------------------------
yad64::address_t ELF64::debug_pointer() {
	read_header();
	...
	for(std::size_t i = 0; i < count; ++i) {
		if(yad64::v1::debugger_core->read_bytes(region_.start() + (section_offset + i * sizeof(Elf64_Phdr)), &section_header, sizeof(Elf64_Phdr))) {
...
//------------------------------------------------------------------------------
yad64::address_t ELF64::calculate_main() {
	...
	for(int i = 0; i < 50; ++i) {
		...
		if(yad64::v1::debugger_core->read_bytes(entry_point + i, &byte, sizeof(byte))) {
			...
			if(ba[0] == 0x48 && ba[1] == 0xc7 && ba[7] == 0xe8 && ba[12] == 0xf4) {
				...
				const yad64::address_t address = *reinterpret_cast<const yad64::address_t *>(ba.data() + 3) & 0xffffffff;
